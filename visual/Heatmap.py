...
def create_heatmap(input_mapping):
    ...
    f = open(input_mapping).read().split('\n')
    if f[-1].strip() == "":
        f = f[:-1]
...
for i in range(100):
    inputFileName=".\\Results\\Animation\\4by5"+str(i)+".txt"
    output = create_heatmap(inputFileName)
    imgplot = plt.imshow(output)
    plt.savefig(".\\Results\\Animation\\Frame"+str(i)+".png")
##plt.show()